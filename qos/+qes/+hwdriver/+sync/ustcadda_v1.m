    ...
    methods
        function Config(obj)
            ...
            for k = 1:obj.numDABoards
                ...
                obj.da_list(k).da.SetOffsetCorr(s.da_boards{k}.daTrigDelayOffset);
        ...
        function isSuccessed = Run(obj,adList) 
            ...
            while(sum(ret) ~= 0)
                ...
                for k = 1:length(adList)
                    [ret(k),obj.ad_list(k).I,obj.ad_list(k).Q] = obj.ad_list(adList(k)).ad.RecvData();
                end
            end
        ...
        function SendWave(obj,channel,data)
            ...
            len = length(data);
            seq = obj.GenerateTrigSeq(len,ch_info.delay);       
            da_struct.da.WriteSeq(ch_info.ch,0,seq);
            if(mod(len,8) ~= 0)
            ...
            da_struct.da.WriteWave(ch_info.ch,0,uint16(data));
            if(mod(floor(da_struct.mask_plus/(2^(ch_info.ch-1))),2) == 0)
                obj.da_list(ch_info.index).mask_plus = da_struct.mask_plus + 2^(ch_info.ch-1);
            end
        end
        ...
        function SetDAChnlOutputOffset(obj,ch,offset)
            if(length(ch) == length(offset))
                for k = 1:length(ch)
                    ...
                    da_struct = obj.da_list(ch_info.index);
                    da_struct.da.SetOffset(ch_info.ch,offset(k));
                end
            else
